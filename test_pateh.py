...
TEST_DATE = date(2026, 7, 25)  # change this to try a different date

with sync_playwright() as pw:
    browser = pw.chromium.launch(headless=False)  # headless=False so you can watch it
    page = browser.new_page()
    results = search_pateh(page, "TBZ", "IST", TEST_DATE)
    browser.close()

print(f"\nFound {len(results)} flights for TBZ->IST on {TEST_DATE}:\n")
for r in results:
